...
// ============================================================
// Валюта: сом (KGS) или рубль (RUB).
// Цены везде хранятся и вводятся в СОМАХ.
// ============================================================
var RUB_PER_KGS = 0.92 // курс: сколько рублей стоит 1 сом — поменяйте здесь при необходимости
var GN_CUR = "kgs"
...
document.addEventListener("DOMContentLoaded", function () {
	...
	if (lb) {
		lb.textContent = GN_LANG === "ky" ? "Русский" : "Кыргызча"
		lb.title = GN_LANG === "ky" ? "Переключить на русский" : "Кыргыз тилине которуу"
		lb.addEventListener("click", function () {
			try { localStorage.setItem("gn_lang", GN_LANG === "ky" ? "ru" : "ky") } catch (e) {}
			location.reload()
		})
	}
	// Переключатель валюты (сом ⇄ рубль)
	var cb = document.getElementById("curBtn")
	if (cb) {
		cb.textContent = GN_CUR === "rub" ? "₽ рубли" : "сомы"
		cb.title = "Поменять валюту (сом / рубль)"
		cb.addEventListener("click", function () {
			try { localStorage.setItem("gn_cur", GN_CUR === "rub" ? "kgs" : "rub") } catch (e) {}
			location.reload()
		})
	}
})
